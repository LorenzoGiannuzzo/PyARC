...
PyARC is a Python-developed software designed for the reconstruction of residential aggregate electrical load profiles. It leverages an algorithm for Association Rule Mining to model complex relationships between Time-of-Use (ToU) data and electrical consumption profiles. The software uses a non-intrusive machine learning methodology that can be used to generate residential electrical consumption profiles at an hourly resolution level using only monthly consumption data (i.e., billed energy). The methodology is mainly composed by three phases: first, identifying the typical load patterns of residential users through k-Means clustering, supported by evaluation metrics to identify the optimal number of clusters such as Davies-Boudin Index, Elbow Method and Silhouette Score, then implementing a Random Forest algorithm, based on features extracted from monthly energy bills, to identify typical load patterns and, finally, reconstructing the hourly electrical load profile through a rescaling factor.
...
- A file .txt containing the accuracy of the RandomForest during training and test, which i saved in the  `docs/User-trained Model Metrics` folder as  `User_trained_model_metrics.txt`. 


 **Reconstruction using Pre-trained Model:**

The output of the reconstruction of the residential aggregate load profile process are:
...
The output of the process are the same as the - **Reconstruction using Pre-trained Model** process, namely:

- A file containing the time series of the aggregate electrical load profile in CSV format, which is saved in the  `data/Output Data` folder structured as follow:
```
Year,Month,Day,Hour,Aggregate load  
2012,1,1,0,5362.737504088515  
2012,1,1,1,4699.1362917288525  
2012,1,1,2,4299.034252481011  
2012,1,1,3,4143.05778396232  
2012,1,1,4,4043.022294386959  
2012,1,1,5,4402.389390089283
...
```
where:
            ...
            -"Aggregate load": contains electrical energy consumption of the residential aggregate expressed in [kWh].
...
```
 ...
 - *Input Data:* This folder contains the required data to use the user-trained, which can be used to reconstruct the eletrical load profile of residential aggregates.

        - `data.csv`: file in CSV format containing the eletrical energy bills on a monthly scale, divided based on the ToU. The file must be structured as follow:
```
User,Year,Month,F1,F2,F3  
MAC000016,2012,1,32.27275,42.496,9.63  
MAC000016,2012,2,7.841,9.5075,5.58
MAC000016,2012,4,3.456,4.502,3.31  
MAC000016,2012,5,2.8345,4.06825,3.46
MAC000016,2012,6,3.504,4.82775,2.8235
...
```
            where:
            - "User": is a char/string which contains usernames;
            - "Year": is a numerical value which respresent the year when eletrical     energy was measured;
            - "Month": is a numerical value which represent the months when electrical energy was measured;
            -"F1,F2,F3": contains electrical energy bills expressed in [kWh] and divided based on the ToU. The column name must be the same the ToU timeframes.
        
		- `tou.csv`: file in CSV format containing the eletrical energy bills on a monthly scale, divided based on the ToU. The file must be structured as follow:
```
  "Hour","ToU"  
0,"F3"  
1,"F3"  
...
```
 ...
 - *Default Training Data:* This folder contains the data used to train the pre-trained  or the pre-trained model, which can be used to reconstruct the eletrical load profile of residential aggregates. The files inside that folder are structured as the files in the `Input Training Data` folder.
 
  - *Centroids*: This folder contains the data containing the information about cluster centroids shape, obtained during the clustering process. The folder cointains a file in CSV format that is structured as follow:

```
Cluster,Hour,Centroid  
1,0,0.2637207555426242  
1,1,0.22434655768686296  
1,2,0.1997015622259806  
1,3,0.18870161496496365  
1,4,0.18341704840807113  
... 
2,0,0.6829219648948441  
2,1,0.6445736349804456  
2,2,0.6290312587566406  
2,3,0.6235405579184536  
2,4,0.6243397833412112  
...
3,0,0.15704705883607284  
3,1,0.13702333683359103  
3,2,0.12460339667704806  
3,3,0.12093336923863841  
3,4,0.11674785616109161  
... 
4,0,0.3947443102687575  
4,1,0.33696615765171994  
4,2,0.30173250234809096  
4,3,0.2840255217189579  
4,4,0.27625352359190597  
...
```
            where:
            - "Cluster" is a numerical value that represent the cluster/subgroup obtained during the clustering process;
            - "Hour": is a numerical value that represent the hours;
            - "Centroids": contains normalized numerical values (range from 0 to 1) which are representative of the normalized eletrical consumption of the respective cluster.
...
*start_program Function:
    ...
    - It prompts the user to choose an action (1. Use pre-trained model, 2. Train a new model, 3. Use user-trained model) and instantiates the `ModelManager` accordingly.
...
This comprehensive class facilitates the end-to-end process of training and utilizing a model for load profile reconstruction and analysis.
     ...
     *aggregate_load(df):*
      ...
      - Aggregates the "load" column for each combination of "Year," "Month," "Day," and "Hour_y."
...
-  `data_clustering.py`: This script defines a class called `Clustering` that provides methods for determining the optimal number of clusters using metrics such as Davies-Bouldin Index, Silhouette Score, and the Elbow Method, and for performing k-means clustering. 
    ...
    *Optimal Cluster Number Determination:
       ...
       - Selects the "M_consumption" column for clustering.
       - Iterates over a range of cluster numbers (4 to 8) and calculates Davies-Bouldin Index, Silhouette Score, and Distortion (Elbow Method).
    ...
    *K-Means Clustering:
       ...
       - Transposes the dataframe into "long" format for plotting.
...
*Method (`normalize_consumption`):
    ...
    - Checks if the 'Consumption' column is present in the DataFrame. If not, raises a `ValueError` with an appropriate message.
    - Verifies if the DataFrame is not empty. If it is empty, raises a `ValueError` with an appropriate message.
...
*Method (`get_negative_values`):
    ...
    - If present, replaces negative values in the "Consumption" column with NaN.
...
*Static Method (`infrequent_profiles`):

    - Iterates through unique day names in the DataFrame.
    - Groups the DataFrame by year, month, and day, creating daily profiles of normalized consumption.
    - Determines the optimal number of clusters for daily profiles using KMeans and Davies-Bouldin Index.
    - Filters out low-populated clusters based on a threshold.
    - Concatenates the updated day DataFrames into a result DataFrame.
...
The `DataPreprocessing` class provides a collection of static methods for various data preprocessing tasks, such as handling negative values, replacing specific daily profiles, interpolating missing values, filling missing values with monthly means, removing outliers, filtering users, calculating monthly averages, reshaping the DataFrame, identifying infrequent profiles, and merging clusters. Each method serves a specific purpose in preparing and cleaning the input DataFrame for further analysis.
...
*Method (get_features2):
    ...
    - Selects numeric columns (excluding "User", "Year", "Month").
